...
function replaceText(word, replacement, text) {
  let item = new RegExp(word, "g");
  let replaceText = text.replace(item, replacement);
  return replaceText;
...
function checkWord(word, text) {
  let item = new RegExp(word, "i");
  let checkWord = item.test(text);
  return checkWord;
...
function extractTextInParentheses(str) {
  let item = /\((.*?)\)/g; //svorotne posilannya
  let matches = str.matchAll(item);
  let findText = [];
  for (let match of matches) {
    findText.push(match[1]);
  }
  return findText;
...
function countEmails(str) {
  ...
  let sum = emails ? emails.length : 0;
  return sum;
...
function checkRegexFlags(regex) {
  const regexFlag = regex.flags;
  const flagG = regexFlag.includes("g");
  const flagM = regexFlag.includes("m");
  return flagG && flagM;
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  let findWord = str.replaceAll(regex, newWord);
  return findWord;
...
function checkFlags(regex) {
  let usingFlag = [];
  if (regex.ignoreCase) {
    usingFlag.push("ignoreCase");
  }
  let flagSource = regex.source;
  usingFlag.push(flagSource);
  return usingFlag;
...
function checkRegexMethods(regex) {
  let usingMetchod = [];
  if (regex.dotAll) {
    usingMetchod.push("dotAll");
  }
  if (regex.multiline) {
    usingMetchod.push("multiline");
  }
  if (regex.sticky) {
    usingMetchod.push("sticky");
  }
  return usingMetchod;
